...
const char programVersion[] = "0.20251225";
...
#define DS18B20_PIN                                                            \
  5 // Connect DS18B20 to D1 pin on NodeMCU v3 (don't forget about the resistor)
...
#define MAXRESULTSCOUNT 100

String sensorAddresses[MAXRESULTSCOUNT]; // String, because we use also
                                         // "DHTtemp" and "DHThumi"; that's also
                                         // good for easy adding future sensors
...
const String TYPE_PERCENT =
    "%"; // correct notation for Polish language (no space before percent sign)
const String TYPE_CDEGREE =
    " °C"; // correct notation for Polish language (space, degree sign, C)

/*
   tables: sensorsDB
*/
// this is the sensors database, it is (will be) stored on the device (in
// EEPROM). Compensation and friendly names.
...
void handleReset() {
  ...
  server.send(
      200, "text/html",
      F("<html><head><meta http-equiv=\"refresh\" "
        "content=\"2;url=/\"></head><body>Reset in progress...</body></html>"));
  sensors.begin(); // ds18b20 init
  blink(4);
}
...
void handleEdit() {
  ...
  // było
  // Serial.println("New name of sensor " + sensorAddressStr + " is " +
  // sensorNewFriendlyNameStr + "; compensation = " +
  // (String)sensorNewCompensation + ".");
...
bool editSensor(const char *sensorAddress, const char *newFriendlyName,
  ...
  if (sensorsCount < MAX_SENSORS) {
    ...
    sensorsSettings[sensorsCount].present =
        false; // nowo dodany sensor jeszcze nie wykryty
    sensorsCount++;
    return true;
  }
...
String sendHTML(uint16_t refresh) {
  ...
  if (sensorResultCount > 0) {
    ptr += "<table>\n";
    for (uint8_t deviceNumber = 0; deviceNumber < sensorResultCount;
         deviceNumber++) {
      ptr += "<tr>\n";
      ptr += "  <td title=" + (String)sensorAddresses[deviceNumber] + ">" +
             (String)friendlyNames[deviceNumber] + "\n";
      ...
      ptr += "  <td title=\"" + (String)compensation[deviceNumber] + "\">" +
             valueString + valueType[deviceNumber] + "\n";
      ptr += "</tr>\n";
    }
    /* nav */
...
String sendXML() {
  ...
  for (uint8_t deviceNumber = 0; deviceNumber < sensorResultCount;
       deviceNumber++) {
    ptr +=
        "<sensor address=\"" + (String)sensorAddresses[deviceNumber] + "\">\n";
    ptr += "  <name>" + (String)friendlyNames[deviceNumber] + "</name>\n";
    ptr += "  <value>" +
           (String)(values[deviceNumber] + compensation[deviceNumber]) +
           "</value>\n";
    ptr += "  <type>" + (String)valueType[deviceNumber] + "</type>\n";
    // ptr += "  <address>" + (String)sensorAddresses[deviceNumber] +
    // "</address>\n";
    ptr += "</sensor>\n";
  }
  ptr += "</report>";
...
deviceNumber++) {
    ...
    ptr += (String)friendlyNames[deviceNumber] + ".value " +
(String)(values[deviceNumber] + compensation[deviceNumber]) + "\n";
  }
  return ptr;
}*/
...
void emptyRestOfTheArray() {
  for (uint8_t sensorIndexInResults = sensorResultCount;
       sensorIndexInResults < MAXRESULTSCOUNT; sensorIndexInResults++) {
...
void updateDHTValues() {
  ...
  if (chk == DHTLIB_OK) {
    ...
    if (!isnan(temperature)) {
      addOrUpdateSensor("DHTtemp", temperature, TYPE_CDEGREE);
    ...
    if (!isnan(humidity)) {
      addOrUpdateSensor("DHThumi", humidity, TYPE_PERCENT);
...
float getTemperature(uint16_t idx) {
  if (!sensorsSettings[idx].present)
    return NAN;
...
void updateDS18B20Values() {
  ...
  Serial.println(String(deviceCount) + " DS18B20 sensors found.");
  ...
  for (uint8_t i = 0; i < deviceCount; i++) {
    ...
    if (temperature != DEVICE_DISCONNECTED_C) {
      addOrUpdateSensor(address, temperature, TYPE_CDEGREE);
      Serial.println("sensor_" + address + "=" + String(temperature));
    } else {
      Serial.println("Skipping sensor_" + address +
                     " (disconnected; error: " + String(temperature) + ").");
    }
  }
}

// dodaje nowy lub aktualizuje istniejący odczyt w tablicach wynikowych
bool addOrUpdateSensor(String address, float value, String sensorType) {
  // szukamy istniejącego odczytu w bieżącej sesji
  for (uint8_t i = 0; i < sensorResultCount; i++) {
    if (sensorAddresses[i] == address) {
      values[i] = value;
      valueType[i] = sensorType;
      return true;
    }
  }
  ...
  // pobieramy dane z bazy sensorów
  uint8_t dbIndex = getSensorsDBIndex(address);
  if (dbIndex != 255) {
    friendlyNames[sensorResultCount] = sensorsSettings[dbIndex].name;
    compensation[sensorResultCount] = sensorsSettings[dbIndex].compensation;
  } else {
    friendlyNames[sensorResultCount] = address;
    compensation[sensorResultCount] = 0;
  }
...
uint8_t getSensorsDBIndex(const String &deviceAddress) {
  for (uint16_t i = 0; i < sensorsCount; i++) {
    if (deviceAddress == String(sensorsSettings[i].address)) {
      return i;
    }
  }
  return 255; // brak w bazie
}
...
bool scanSensors() {
  ...
  for (uint8_t i = 0; i < found; i++) {
    ...
    if (idx >= 0) {
      ...
      strlcpy(sensorsSettings[sensorsCount].address, addrStr,
              sizeof(sensorsSettings[sensorsCount].address));
      snprintf(sensorsSettings[sensorsCount].name,
               sizeof(sensorsSettings[sensorsCount].name), "Sensor %u",
               sensorsCount + 1);
